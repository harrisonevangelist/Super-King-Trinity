function pawnPaths(x, y, isWhite = true) {
  ...
  if (onBoard(oneStep) && !isObstacle(oneStep)) {
    if (oneStep[1] === promotionRank) {
      for (let promo of ['Q', 'R', 'N', 'B', 'P']) {
        const score = pieceValues[promo] + getScore(oneStep);
        paths.push({ path: [[x, y], oneStep], promotion: promo, score });
      }
    } else {
      paths.push({ path: [[x, y], oneStep], score: getScore(oneStep) });
    }

    const twoStep = [x, y + 2 * direction];
    if (y === startRank && onBoard(twoStep) && !isObstacle(twoStep)) {
      paths.push({ path: [[x, y], twoStep], score: getScore(twoStep) });
    }
  }
  ...
  for (let dx of [-1, 1]) {
    ...
    if (onBoard(diag) && !isObstacle(diag)) {
